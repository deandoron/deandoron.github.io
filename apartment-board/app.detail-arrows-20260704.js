...
function renderThumbs() {
  ...
  for (const imageId of room.imageIds) {
    ...
    number.textContent = thumbNumber;
...
function renderDetail() {
  ...
  const roomImageIds = getActiveRoomImageIds();
  if (roomImageIds.length > 1) {
    const prevButton = createDetailNavButton("<", "Previous image", -1);
    prevButton.classList.add("detail-nav-prev");

    const nextButton = createDetailNavButton(">", "Next image", 1);
    nextButton.classList.add("detail-nav-next");

    imageWrap.append(prevButton, nextButton);
  }
...
function createDetailNavButton(symbol, label, direction) {
...
function selectAdjacentImage(direction) {
  const imageIds = getActiveRoomImageIds();
  if (imageIds.length <= 1) return;

  const currentIndex = imageIds.indexOf(state.activeImageId);
  const nextIndex =
    currentIndex >= 0
      ? (currentIndex + direction + imageIds.length) % imageIds.length
      : 0;

  state.activeImageId = imageIds[nextIndex];
  queueStateSave();
  renderThumbs();
  renderDetail();
}
...
function getActiveRoomImageIds() {
  const room = getActiveRoom();
  if (!room) return [];
  return room.imageIds.filter((id) => imageRecords.has(id));
}
